...
The `dc_imed1/2` columns complete that pair with the per-tracker **median operating current**. `dc_current1/2` above is the interval *minimum* — deliberately, to catch the most-throttled instant for curtailment — and that statistic cannot double as a shading measure: one passing cloud inside the half hour pins the slot near zero, and the failure is worst at low sun, precisely where a shading signal is wanted. Measured on a live 74-day store, the minimum left **56 of 67 mornings with no usable 08:00 reading**, while a median reducer over the same per-second series recovered them. With both medians present the mechanism reads directly: current down with voltage held at Vmp is a shadow line lying evenly across every module (a *linear* loss in unshaded area); voltage down as well is bypass-diode conduction (non-linear, and not representable by a multiplicative transmission factor). Forward-only — a minimum cannot be reconstructed into a median — so history starts at upgrade, and nothing consumes the columns yet.

**`_total` tracker fallback.** The property-wide row's DC capture reads the flat single-inverter `CONF_MPPT*` keys, which the sites step clears when a multi-array config is saved (each array then maps its own trackers). `_property_mppt_list` therefore falls back to the configured arrays' trackers, truncated to `MAX_MPPT_TRACKERS`; without it the `_total` row's DC columns are written once at setup and then stay zero forever on every multi-array install, while the per-site rows fill normally. The gap was invisible on the **MPPT DC** diagnostic sensor because its `max_voltage` already spanned the per-site trackers.
...
`discover_sites(hass)` (shared by coordinator and config flow) enumerates the base RooftopSensors, reading `resource_id`, `name`, `capacity`, `capacity_dc`, `tilt`, `azimuth`, `compass_degrees`. A rooftop is recognised by its **entity-registry platform** (`solcast_solar`), never by its entity id: the base's `RooftopSensor` has never set `has_entity_name`, so the id is `sensor.<site>` on cores before HA 2026.4 and only the device-prefixed `sensor.solcast_pv_forecast_<site>` on later ones, and the registry keeps whichever form the entity was created with. The former `"solcast" in entity_id` filter dropped every pre-2026.4 rooftop, returned zero sites and silently skipped the sites step (discussion #65). The substring test remains only for states with no registry entry (test fixtures). Orientation seeds per-site tuning; `resource_id` keys storage and targets `set_dampening`.

`CONF_SITE_GROUPS` is a list of measurement groups — either a single-site group (`site` + `ac_sensor`) or a DC-apportioned group (a `strings` list of `{site, dc_sensors}`), each optionally carrying an `mppts` list of per-tracker voltage/current capture sensors. `dc_sensors` is a **list**, so one array wired across several MPPTs sums them before its share is taken; the single-entity `dc_sensor` key written by older versions is still read. Note the two DC roles are distinct: `dc_sensors` is **power** (apportionment ratio only); `mppts` is **instantaneous voltage/current** (curtailment capture).

**Topology selector (`CONF_SITE_TOPOLOGY`).** The `sites` step leads with an explicit measurement-topology choice rather than inferring it from the fields, because Solcast data carries no inverter-type signal. `direct` (microinverters / one inverter per array) renders no DC field and `_derive_groups(mode="direct")` makes every mapped array its own single-site group. `dc_split` (one shared inverter, multiple MPPTs) renders the DC field and `_derive_groups(mode="dc_split")` groups arrays sharing an AC sensor into one apportioned group. `shared_no_dc` (one combined meter with no per-array telemetry — a Tesla Powerwall 3, a single revenue meter) renders *only* the selector and derives an empty group list: with one summed reading there is no per-array signal to recover, and apportioning by forecast share would be circular (`actualᵢ/Fᵢ = ΣA/ΣF` for every array, i.e. exactly the property-wide ratio relabelled), so the property is measured as a single aggregate whose hourly dampening curve still separates a morning-facing array from an afternoon-facing one in time. The mode is persisted; for entries saved before this selector existed, `_infer_topology()` derives the default (`strings` present ⇒ `dc_split`, else `direct`). Submissions are validated up front per mode by `_validate_assignments()`: `dc_split` via `_validate_dc_split()` — a missing per-array DC sensor or non-identical AC sensors surface a form error (`dc_split_missing_dc` / `dc_split_ac_mismatch`) instead of the array being silently omitted; `direct` via `_validate_direct()`, which rejects the same entity mapped to two arrays (`direct_ac_duplicate`). That last case is the shared-meter install reaching for the only sensor it has, and it fails destructively when accepted: each array records the whole property, its actual/forecast ratio reads far above 1, and the `[0, 1]` clamp in `_push_dampening` turns that into *no dampening at all* — on a base already flipped into granular per-site mode by the push. For the same reason `default_ac` seeds the generation field only in `dc_split`, never in `direct`. One topology per property (mixed shared/independent installs are out of scope; a hand-authored `CONF_SITE_GROUPS` still supports mixed). A change of mode on submit re-renders the step with the matching fields, preserving entries.
