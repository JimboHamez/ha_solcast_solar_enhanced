...
`CONF_SITE_GROUPS` is a list of measurement groups — either a single-site group (`site` + `ac_sensor`) or a DC-apportioned group (a `strings` list of `{site, dc_sensor}`), each optionally carrying an `mppts` list of per-tracker voltage/current capture sensors. Note the two DC roles are distinct: `dc_sensor` is **power** (apportionment ratio only); `mppts` is **instantaneous voltage/current** (curtailment capture).
