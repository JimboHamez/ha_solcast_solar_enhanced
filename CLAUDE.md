...
| `coordinator.py` | `SolcastEnhancedCoordinator` (DataUpdateCoordinator) — half-hour-aligned update loop; orchestrates store writes, PV tuning (24 h), dampening push (6 h timer + on base forecast poll), OWM fetch |
| `sensor.py` | 16 `CoordinatorEntity` sensors; all read from coordinator data/properties. The three 30-min average sensors (PV Power/Export, Battery Charge) also extend `RestoreSensor` via `_RestoringSensorBase` to survive restarts. The diagnostic `MpptDcSensor` surfaces the latest captured per-MPPT DC telemetry. The diagnostic enum `OrientationRecommendationSensor` exposes the divergence-gated "update your Solcast tilt/azimuth?" status (`matches` / `update_suggested` / `insufficient_data`) — the actionable counterpart to the raw Tuned Panel Tilt/Azimuth angles |
...
6. On 24 h timer: run `pv_tuning.run_tuning()` in a thread executor (aggregate `_total`, then per-site). The fit spans the **full history** (`TUNING_MAX_RECORDS` cap, default 20000 — a memory guard, not a window) rather than a recent slice: panel orientation is a fixed constant, so a full-history fit cancels seasonal sun-angle bias and *converges* instead of drifting day to day. The `orientation_recommendation` property turns that converged fit into a divergence-gated status (tighter `RECOMMEND_TILT_TOL`/`RECOMMEND_AZIMUTH_TOL` than the dampening gate) so the user is nudged once when their Solcast config is genuinely off, not asked to chase a live number
7. On 6 h timer **or** shortly after the base polls a fresh forecast (`sensor.solcast_pv_forecast_api_last_polled` advances, debounced by a grace delay): compute 48 half-hour dampening slots → average to 24 hourly → push via `solcast_solar.set_dampening` service call (per-site when multi-site groups are configured)
...
- Poll-driven dampening trigger: a state-change listener on `sensor.solcast_pv_forecast_api_last_polled` (`BASE_API_LAST_POLLED_SENSOR`) re-pushes dampening shortly after the base fetches a fresh forecast, so the latest curve lands on the new data instead of waiting for the 6 h timer. Registered in `async_setup` only when `CONF_AUTO_DAMPENING` is on; the current value is seeded first so an existing state doesn't fire at setup. `_handle_api_poll` ignores no-op/unusable states and, after a `DAMPENING_POLL_DELAY_SECONDS` grace delay (lets the base parse/store the `detailedForecast` detail we read; also debounces poll-sensor bursts via `async_call_later`), runs `_async_dampening_on_poll` → `_run_dampening`, which also resets `_last_dampening_ts` so the periodic timer doesn't immediately repeat it. Both listener handles are torn down in `async_teardown`.
