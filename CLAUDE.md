...
| `sensor.py` | 16 `CoordinatorEntity` sensors; all read from coordinator data/properties. The three 30-min average sensors (PV Power/Export, Battery Charge) also extend `RestoreSensor` via `_RestoringSensorBase` to survive restarts. The diagnostic `MpptDcSensor` surfaces the latest captured per-MPPT DC telemetry; `PvForecastConfidenceSensor` surfaces the load-scheduling confidence (item 3) |
