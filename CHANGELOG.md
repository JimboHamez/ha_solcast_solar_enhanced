...
> one combined meter, and refuses a mapping that silently disabled dampening.
...
> forward-only. Nothing consumes the new data yet, and tuning, dampening and the
> pushed factors are unchanged.
...
> diluting measured shading in the best hours of the day.
...
Housekeeping against the Home Assistant [Integration Quality Scale](https://developers.home-assistant.io/docs/core/integration-quality-scale/checklist) — every applicable Bronze, Silver and Gold rule now passes. No behavioural change to tuning, dampening or storage.

### Added
- **Diagnostics download** (`diagnostics`). **⋮ → Download diagnostics** on the integration page returns the effective configuration, the last collector readings (weather, irradiance, discovered and configured arrays), the raw 48-slot dampening curve plus each array's, the tuning fit, the store's record count and coverage, and the confidence state. The OpenWeatherMap key and the site coordinates are redacted; a test asserts the key cannot appear anywhere in the payload, not merely at the field it was read from. The half-hour curve is included rather than the hour-averaged sensor attributes, because the half-hour grid is what gets averaged and pushed — the averaged view would hide a slot-level asymmetry. Gathered by a new `coordinator.diagnostics_snapshot()` so `diagnostics.py` doesn't reach into private attributes.
- **A reconfigure flow** (`reconfiguration-flow`). The whole six-step wizard can be re-run against an existing entry from the integration menu, seeded with its current settings so each step opens on what is configured today. The collected settings are written to **both** `data` and `options`: the coordinator reads `{**data, **options}`, so writing only `data` would leave a value stored by an earlier options-flow run shadowing the one just entered, and the reconfigure would appear to do nothing. Pinned by a test.
- **Stale per-array devices are cleaned up** (`stale-devices`). Dropping an array in the sites step removed its entities but left its device behind as an empty card. Devices whose array is no longer configured are now detached on setup, and `async_remove_config_entry_device` lets you delete one by hand — refused for arrays still configured, so Home Assistant won't offer to delete a live device. The entry is detached from the device rather than the device deleted outright, so a device another integration also claims survives.
- **README sections for examples, troubleshooting and known limitations** (`docs-examples`, `docs-troubleshooting`, `docs-known-limitations`). Copy-paste automations (confidence-gated load scheduling, forced re-tune, a shading-loss template, the orientation advisory, debug logging); a symptom → cause → fix table led by the diagnostics download; and a plain statement of the limits — tilt frequently non-identifiable and azimuth never tuned, the winter α ramp suppressing real shading, the GHI-based clear-sky gate admitting beam-poor days, the undampened denominator not being backfillable past ~28 days, per-site forecasts sometimes being apportioned rather than measured, the coupling between our push and the base's granular dampening, and battery support never having run against real hardware.
...
- **A quality-scale badge and self-assessment section** in the README. The badge reports Gold and links to a section stating plainly that this is our own audit, not an awarded tier — the quality scale is a programme for integrations shipping inside Home Assistant Core, and a custom integration is not eligible for an official rating. The section tables what passes and what doesn't at every tier, so the claim can be checked rather than taken on trust.
- **`icons.json`** (`icon-translations`) — the 22 sensor icons moved out of `_attr_icon` into an icon-translation file, and the three actions gained icons of their own (shown in the action picker). Home Assistant resolves them by translation key, which also means a future release could vary an icon by state without touching Python. Weather Temperature is deliberately absent: it carries a temperature device class and inherits an icon from it.
- **`entity.py`** (`common-modules`) — the base entity classes moved out of `sensor.py` into a dedicated module, and were renamed from private to public in the process: `_EnhancedSensorBase` → `SolcastEnhancedEntity`, `_RestoringSensorBase` → `RestoringSensorEntity`, `_SiteSensorBase` → `SolcastEnhancedSiteEntity`. No behavioural change; the classes are unchanged apart from the move.
...
- **Actions now report a failure instead of swallowing it** (`action-exceptions`). A failure inside `run_pv_tuning`, `run_dampening_update` or `fetch_weather` — a locked database, a dead network — was written to the log and otherwise discarded, so the action reported success and an automation built on it carried on as if the work had happened. Each now raises a translated `HomeAssistantError` naming what failed, with the original exception kept as the cause. An error the coordinator has already phrased for the user (a `ServiceValidationError`) passes through untouched rather than being re-wrapped in the generic message.
- **Entities that have lost their data source read *unavailable*** (`entity-unavailable`). *Weather Temperature* and *Cloud Cover* go unavailable when no weather source is enabled or a fetch returns nothing; *MPPT DC Voltage* goes unavailable when no per-string DC sensors are configured. Previously each sat at *unknown* indefinitely, which is indistinguishable at a glance from a real reading that hasn't arrived yet.

  Deliberately **not** applied to the per-array sensors or **Tuned Tilt**. A per-array value that hasn't been measured yet is genuinely unknown, not unsourced. And Home Assistant hides the attributes of an unavailable entity — for Tuned Tilt those attributes carry the reason the tilt was rejected and the value the fit produced, which is the entire diagnostic. (The b9 notes and docstring described that state as "unavailable"; it was always *unknown*, and the wording is now corrected rather than the behaviour.)
- **`PARALLEL_UPDATES = 0` is declared** (`parallel-updates`). Every entity is read-only and fed from the single coordinator refresh, so there is nothing to serialise.
- **The Storage, Battery and Tuning step schemas are now built once and shared** by the config, options and reconfigure flows, as the Weather step already was. The config flow previously carried its own copy with plain defaults while the options flow carried a near-identical copy reading current values — three duplicated schemas that had to be kept in step by hand. Each builder now takes a `current` mapping, which is empty on a first-run setup and so yields exactly the previous defaults. Internal; no change to what any step displays on a fresh install.

### Fixed
- **`config_flow.py` now has 100% test coverage** (`config-flow-test-coverage`). Five new tests cover the per-site helper edge cases that were unreached: an array left without a generation sensor in either derive path, the flat-MPPT migration declining to overwrite trackers the user already mapped, duplicate Solcast site names being disambiguated into distinct fields, and the options flow reporting an incomplete DC split. The unreachable `EntitySelector` import guard was removed rather than tested around — `homeassistant.helpers.selector.EntitySelector` has existed since long before the declared 2026.5.4 minimum, so the fallback could never fire.
- **`fetch_weather` now refreshes every configured source, not just OpenWeatherMap.** The action only ever called the OWM client, so on a default install — where Open-Meteo is the keyless source and OWM is not configured — it did nothing at all while still reporting success. It now runs the same fetch the half-hourly cycle does (OWM if configured, Open-Meteo irradiance and its cloud/temperature fallback), and raises a `ServiceValidationError` telling you to enable a source when neither is on. The shared code path also removes the duplicate that let the two drift apart in the first place.
- Test coverage rose to **96%** overall, past the 95% the scale requires (`__init__.py` 92% → 100%, `sensor.py` 93% → 100%, and a new `tests/test_entity.py` takes `entity.py` 77% → 100%). Four tests pin the naming contract: no sensor may carry a raw `_attr_name`, every declared translation key must resolve to a name, `strings.json` may carry no orphaned names, and all 11 locales must define a non-empty name for every key. The new per-array sensor tests assert the **site id each sensor queried with**, not just that a number came back — a sensor that ignored its own `_site_id` and read the property-wide value would have passed the weaker check.
...
### Documentation
- **Added the config-wizard and dashboard screenshots to the README.** Each of the six
  wizard steps now shows a (redacted) screenshot — Step 6 captured on 1.9.1 so it reflects
  the corrected topology-selector label — laid out as click-to-enlarge thumbnails with the
  field tables alongside, plus a dashboard hero image. No code changes.
