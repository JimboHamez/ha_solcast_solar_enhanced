...
**PV tuning fits clear-sky data across all seasons, and the dampening push no longer fails.** Tuning previously took the most recent 2000 rows *of any weather* and only then filtered for clear sky — so in a cloudy season it could be left fitting a tiny winter sample while thousands of clear-sky rows sat unused in older data. The clear-sky filter now runs **in SQL before the limit**, so the tuner uses up to 2000 clear-sky records spanning summer (high sun) and winter (low sun) — the angular diversity that actually pins down tilt and azimuth. Separately, the **dampening push no longer fails** when a computed factor exceeds 1.0 (an hour where measured output beats the forecast): factors are clamped to the `[0,1]` range the base `set_dampening` service requires, with the true value kept in the sensor attributes. See the [release notes](https://github.com/JimboHamez/ha_solcast_solar_enhanced/releases/tag/v1.6.6) and [CHANGELOG](CHANGELOG.md).

_Previously, in v1.6.5:_ fixed the **panel azimuth convention** mismatch — the configured azimuth (Solcast West-positive) is now converted to the tuner's internal frame at every seed point and the tuned azimuth is reported back in your convention; `tools/import_history.py` also recomputes zenith as well as azimuth. **Re-read your Tuned Panel Tilt/Azimuth sensors after upgrading from ≤1.6.4.**
