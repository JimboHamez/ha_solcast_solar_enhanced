...
**Export curtailment no longer looks like shading to the dampening calculation.** On a sunny day a site whose PV exceeds household load plus its grid export limit gets *curtailed* — the inverter holds output below what the panels could make — so `pv_actual` falls short of the forecast for a reason that isn't shading. Dampening previously read that shortfall as a shading penalty and pushed a spuriously low factor on exactly the clear-sky days it depends on (the tuning side already excluded these records; dampening didn't). Dampening now **clips the forecast to the achievable ceiling** (`delivered output + remaining export headroom`, floored so the ratio never exceeds 1.0): a curtailed record contributes a neutral ≈1.0 instead of a false penalty, and **no record is discarded**. The export limit is read from the base integration's `site_export_limit` (manual `export_limit_kw` fallback; `0` disables it, so it's a no-op for unconstrained sites). A per-hour `forecast_clipped` count appears on the Dampening sensor. This most affects sites with an export limit below their array's clear-sky peak — and will matter more widely as variable export limits and curtailment schemes roll out. See the [release notes](https://github.com/JimboHamez/ha_solcast_solar_enhanced/releases/tag/v1.6.7) and [CHANGELOG](CHANGELOG.md).

_Previously, in v1.6.6:_ PV tuning fits the most recent **clear-sky** records across all seasons (the clear-sky filter runs in SQL before the row limit, instead of after), and the **dampening push no longer fails** when a factor exceeds 1.0 — factors are clamped to the `[0,1]` range the base `set_dampening` requires, with the true value kept in the sensor attributes.
