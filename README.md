...
**PV tuning panel azimuth now uses the correct (Solcast) convention.** The base integration and Solcast express panel azimuth as degrees from North with **West positive, East negative** (0=N, +90=W, −90=E), but the tuner's internal solar maths is **East positive** — and the two weren't being converted. So the configured azimuth was seeded into the optimiser mirrored East↔West (which also distorted the tilt estimate), and **Tuned Panel Azimuth** was displayed in the wrong frame. Tuning now converts at every seed point and reports the result back in the convention you entered, so it's directly comparable to your Solcast site. **Re-read your Tuned Panel Tilt/Azimuth sensors after upgrading.** Also: `tools/import_history.py` now recomputes **zenith** as well as azimuth from each row's epoch (normalising history collected with a different sun-position source/sampling time) and creates the destination directory if missing. See the [release notes](https://github.com/JimboHamez/ha_solcast_solar_enhanced/releases/tag/v1.6.5) and [CHANGELOG](CHANGELOG.md).

_Previously, in v1.6.4:_ PV tuning **dropped the scipy dependency** for a pure-**numpy grid search** so it works on a Raspberry Pi, gained a **per-site dampening convergence gate**, an optional **history retention** setting, a multi-site crash fix when OpenWeatherMap is absent, and **translations for 10 new languages**.
